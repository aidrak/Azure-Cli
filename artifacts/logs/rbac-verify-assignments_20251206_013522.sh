...
if [[ -n "$FILE_SHARE_ID" ]]; then
  ...
  if [[ -n "$ASSIGNMENT" ]]; then
    ...
    echo "[WARNING] ⚠ Standard users: Storage File Data SMB Share Contributor (assignment in progress)"
    ((WARNING_CHECKS++))
  fi
  ...
  if [[ -n "$ASSIGNMENT" ]]; then
    ...
    echo "[WARNING] ⚠ Admin users: Storage File Data SMB Share Elevated Contributor (assignment in progress)"
    ((WARNING_CHECKS++))
  fi
else
...
if [[ -n "$ASSIGNMENT" ]]; then
  ...
  echo "[WARNING] ⚠ Standard users: Virtual Machine User Login (assignment in progress)"
  ((WARNING_CHECKS++))
fi
...
if [[ -n "$ASSIGNMENT" ]]; then
  ...
  echo "[WARNING] ⚠ Admin users: Virtual Machine Administrator Login (assignment in progress)"
  ((WARNING_CHECKS++))
fi
...
if [[ -n "$APP_GROUP_ID" ]]; then
  ...
  if [[ -n "$ASSIGNMENT" ]]; then
    ...
    echo "[WARNING] ⚠ Standard users: Desktop Virtualization User (assignment in progress)"
    ((WARNING_CHECKS++))
  fi
  ...
  if [[ -n "$ASSIGNMENT" ]]; then
    ...
    echo "[WARNING] ⚠ Admin users: Desktop Virtualization User (assignment in progress)"
    ((WARNING_CHECKS++))
  fi
else
