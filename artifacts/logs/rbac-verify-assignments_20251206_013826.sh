...
if [[ -n "$STORAGE_ACCOUNT" && "$STORAGE_ACCOUNT" != "null" ]]; then
  STORAGE_ID=$(az storage account show \
    ...
    --query id -o tsv)
  FILE_SHARE_ID="${STORAGE_ID}/fileServices/default/fileshares/fslogix-profiles"
else
...
echo "[DEBUG] FILE_SHARE_ID: [$FILE_SHARE_ID]"
...
if [[ -n "$FILE_SHARE_ID" ]]; then
  ...
  ASSIGNMENT=$(az role assignment list \
    ...
    --scope "$FILE_SHARE_ID" \
    --query "[?roleDefinitionName=='Storage File Data SMB Share Contributor']" -o tsv || true)
  if [[ -n "$ASSIGNMENT" ]]; then
    echo "[SUCCESS] [OK] Standard users: Storage File Data SMB Share Contributor"
    ((PASSED_CHECKS++))
  else
    echo "[WARNING] [WAIT] Standard users: Storage File Data SMB Share Contributor (assignment in progress)"
    ((WARNING_CHECKS++))
  fi
  ...
  ASSIGNMENT=$(az role assignment list \
    ...
    --scope "$FILE_SHARE_ID" \
    --query "[?roleDefinitionName=='Storage File Data SMB Share Elevated Contributor']" -o tsv || true)
  if [[ -n "$ASSIGNMENT" ]]; then
    echo "[SUCCESS] [OK] Admin users: Storage File Data SMB Share Elevated Contributor"
    ((PASSED_CHECKS++))
  else
    echo "[WARNING] [WAIT] Admin users: Storage File Data SMB Share Elevated Contributor (assignment in progress)"
    ((WARNING_CHECKS++))
  fi
else
  echo "[WARNING] [SKIP] Storage checks skipped (no storage account found)"
fi
...
ASSIGNMENT=$(az role assignment list \
  --assignee "$STD_GROUP_ID" \
  --scope "$RG_ID" \
  --query "[?roleDefinitionName=='Virtual Machine User Login']" -o tsv || true)
if [[ -n "$ASSIGNMENT" ]]; then
  echo "[SUCCESS] [OK] Standard users: Virtual Machine User Login"
  ((PASSED_CHECKS++))
else
  echo "[WARNING] [WAIT] Standard users: Virtual Machine User Login (assignment in progress)"
  ((WARNING_CHECKS++))
fi
...
ASSIGNMENT=$(az role assignment list \
  --assignee "$ADMIN_GROUP_ID" \
  --scope "$RG_ID" \
  --query "[?roleDefinitionName=='Virtual Machine Administrator Login']" -o tsv || true)
if [[ -n "$ASSIGNMENT" ]]; then
  echo "[SUCCESS] [OK] Admin users: Virtual Machine Administrator Login"
  ((PASSED_CHECKS++))
else
  echo "[WARNING] [WAIT] Admin users: Virtual Machine Administrator Login (assignment in progress)"
  ((WARNING_CHECKS++))
fi
...
ASSIGNMENT=$(az role assignment list \
  --assignee "$ADMIN_GROUP_ID" \
  --scope "$RG_ID" \
  --query "[?roleDefinitionName=='Desktop Virtualization Power On Off Contributor']" -o tsv || true)
if [[ -n "$ASSIGNMENT" ]]; then
  echo "[SUCCESS] [OK] Admin users: Desktop Virtualization Power On Off Contributor"
  ((PASSED_CHECKS++))
else
  echo "[WARNING] [WAIT] Admin users: Desktop Virtualization Power On Off Contributor (optional)"
  ((WARNING_CHECKS++))
fi
...
if [[ -n "$APP_GROUP_ID" ]]; then
  ...
  ASSIGNMENT=$(az role assignment list \
    --assignee "$STD_GROUP_ID" \
    --scope "$APP_GROUP_ID" \
    --query "[?roleDefinitionName=='Desktop Virtualization User']" -o tsv || true)
  if [[ -n "$ASSIGNMENT" ]]; then
    echo "[SUCCESS] [OK] Standard users: Desktop Virtualization User"
    ((PASSED_CHECKS++))
  else
    echo "[WARNING] [WAIT] Standard users: Desktop Virtualization User (assignment in progress)"
    ((WARNING_CHECKS++))
  fi
  ...
  ASSIGNMENT=$(az role assignment list \
    ...
    --query "[?roleDefinitionName=='Desktop Virtualization User']" -o tsv || true)
  if [[ -n "$ASSIGNMENT" ]]; then
    echo "[SUCCESS] [OK] Admin users: Desktop Virtualization User"
    ((PASSED_CHECKS++))
  else
    echo "[WARNING] [WAIT] Admin users: Desktop Virtualization User (assignment in progress)"
    ((WARNING_CHECKS++))
  fi
else
  echo "[WARNING] [SKIP] Application group checks skipped (no app group found)"
fi
